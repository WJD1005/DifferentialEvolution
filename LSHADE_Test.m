function [minError, errorTrace] = LSHADE_Test(initNP, D, maxFES, p, Asize, H, searchRange, fhd, funcNum, realMinVal)
%LSHADE_TEST L-SHADE算法测试函数，用于测试算法性能。
% 输入：
...
errorTrace(1) = min(xCost) - realMinVal;  % 储存每代最小误差值
...
while FES < maxFES
    ...
    errorTrace(end + 1) = min(xCost) - realMinVal;
end
